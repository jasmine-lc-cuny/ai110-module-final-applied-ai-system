...
- The app was narrowed to dogs and cats only (the exotic-species groups — small pets, avians, reptiles/amphibians, fish/invertebrates — were removed as out of scope); `pawpal_system.pet_species_icon()` and the Patients/Pet Profile pages were simplified accordingly, and the demo-data seeder now only generates dogs and cats. Verified via `AppTest` on both pages plus the full pytest suite.
